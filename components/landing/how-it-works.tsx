          ...
          <div key={s.title} className="relative">
            <div className="flex h-full flex-col rounded-2xl border bg-card p-6 transition-shadow hover:shadow-card">
              <div className="flex items-center justify-between">
                ...
                <div className="flex h-9 w-9 items-center justify-center rounded-lg border bg-background text-foreground">
            ...
            {i < steps.length - 1 && (
              <ArrowRight className="absolute right-[-18px] top-1/2 hidden h-4 w-4 -translate-y-1/2 text-muted-foreground md:block" />
            )}
          </div>
        ))}
      </div>
